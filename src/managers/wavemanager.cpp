...
void WaveManager::PreKillWave(const EnemyWave wave) {
  mgr_message->InsertMessage(
      TempMessageKeys::Mes_WaveCounter,
      TempMessage{"Cleared: $" + std::to_string(wave.p_moneyReward) + "+", 3.0f,
                  3.0f});
...
void WaveManager::RemoveUnactiveWave(const uint32_t id) {
  ...
  m_unactiveWaves.erase(std::remove_if(m_unactiveWaves.begin(),
                                       m_unactiveWaves.end(), eraseLambda),
                        m_unactiveWaves.end());
}
void WaveManager::RemoveActiveWave(const uint32_t id) {
  ...
  m_activeWaves.erase(
      std::remove_if(m_activeWaves.begin(), m_activeWaves.end(), eraseLambda),
      m_activeWaves.end());
}
...
void WaveManager::ClearActiveWaves() { m_activeWaves.clear(); }
void WaveManager::ClearUnactiveWaves() { m_unactiveWaves.clear(); }
void WaveManager::Update(const float deltaTime) {
  UpdateUnactiveWaves();
  UpdateActiveWaves(deltaTime);
}
void WaveManager::InjectDependencies(EnemyManager *enemyManager,
                                     PSManager *psManager,
...
void WaveManager::Init() {
  assert(mgr_enemy != nullptr &&
         "enemyManager wasn't injected and its dependency");
  assert(mgr_ps != nullptr && "psManager wasn't injected and its dependency");
  assert(mgr_tower != nullptr &&
         "towerManager wasn't injected and its dependency");
  assert(mgr_message != nullptr &&
         "messageManager wasn't injected and its dependency");
